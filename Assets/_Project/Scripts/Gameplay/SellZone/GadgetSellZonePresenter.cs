...
namespace Gameplay.Gadgets
{
    ...
    public class GadgetSellZonePresenter : MonoBehaviour
    {
        ...
        [SerializeField] private float _maxTrackingDistance = 3.0f; // Your value of 3
        ...
        private void UpdateSpringTargetToTrackGadget()
        {
            ...
            if (Vector3.Distance(anchorCorner, closestPointOnGadget) > _maxTrackingDistance)
            {
                StopAllProcesses();
                return;
            }

            Vector3 targetCorner = _currentTarget.ObjectCollider.bounds.min - new Vector3(_capturePadding, _capturePadding, 0);
